...
import json
from src.utils.pcd_utils import generate_pcds
from src.utils.data_utils import extract_frame_numbers
...
parser.add_argument("--eps", type=float, default=0.03, help="Distance threshold for clustering")
parser.add_argument("--min_points", type=int, default=15, help="Minimum number of points in a cluster")
parser.add_argument("--nb_neighbors", type=int, default=15, help="Number of neighbors to consider for statistical outlier removal")
...
for obj_key in cache:
    ...
    if not os.path.isdir(object_input_dir):
        ...
        print(bcolors.OKGREEN + f"Object input directory already found for {obj_key}" + bcolors.ENDC)
...
for obj_key in cache:
    ...
    if not os.path.isdir(os.path.join(object_dir, "images")):
        ...
        print(bcolors.OKGREEN + f"Masks successfully created for {obj_key}" + bcolors.ENDC)
    else:
        print(bcolors.OKGREEN + f"Masks directory already found for {obj_key}" + bcolors.ENDC)

with open(cache_path, 'w') as f:
    json.dump(cache, f, indent=4)

## Cache every 5th fram number for each obejct
extract_frame_numbers(f"data/{args.scene_name}")

## Generate PCDs for each object
pcds_path = f"data/{args.scene_name}/pcds"
if not os.path.isdir(pcds_path):
    print(bcolors.OKCYAN + f"Making point cloud for each object"+ bcolors.ENDC)
    generate_pcds(args.scene_name, eps=args.eps, min_points=args.min_points, 
                    ...
                    save_frames=None, load_cached_frames=True)
    print(bcolors.OKGREEN + f"Point cloud successfully generated"+ bcolors.ENDC)
else:
    print(bcolors.OKGREEN + f"Point cloud directory already found, skipping PCD generation"+ bcolors.ENDC)

## Generate template URDF
print(bcolors.OKCYAN + f"Making template urdf"+ bcolors.ENDC)
pcd_data, center, labels, label_keywords = prepare_pcd_data(pcds_path, save_labels=False, load_cached_labels=True)
pcd_to_urdf_simple_geometries(pcd_data, center, labels, scene_name=args.scene_name)
print(bcolors.OKGREEN + f"Template successfully generated"+ bcolors.ENDC)
